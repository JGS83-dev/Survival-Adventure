...
public class Warp : MonoBehaviour {
	//Almacenar el punto de objetivo
	public GameObject objetivo;
	//Almacenar el mapa de objetivo
	public GameObject objMapa;
	...
	IEnumerator OnTriggerEnter2D (Collider2D col) {
		...
		if (col.CompareTag ("Player")) {
			...
			col.GetComponent<Mov> ().enabled = false;
			...
			col.transform.position = objetivo.transform.GetChild (0).transform.position;
			Camera.main.GetComponent<SegCamara>().setBound(objMapa);
			FadeOut ();
			col.GetComponent<Animator> ().enabled = true;
			col.GetComponent<Mov> ().enabled = true;

			// Por último mostramos el nombre de la zona 2 segundos
			StartCoroutine(area.GetComponent<Area>().ShowArea(objMapa.name));
		}
	}
